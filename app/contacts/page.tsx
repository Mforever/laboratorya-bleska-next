...
export default function ContactsPage() {
  ...
  const contactCards = [
    { id: 'address', icon: 'fa-map-marker-alt', title: 'Адрес', lines: ['Омск, Чкаловский м/р', 'ул. Индустриальная, 5Б'], workingHours: 'Ежедневно: 10:00–20:00', workingByAppointment: true, buttonText: 'Открыть на карте', buttonLink: 'https://yandex.ru/maps/org/laboratoriya_bleska/224582731097/', buttonAction: undefined },
    { id: 'phone', icon: 'fa-phone-alt', title: 'Телефон', lines: ['+7 (962) 055-58-58'], buttonText: 'Позвонить', buttonLink: 'tel:+79620555858', buttonAction: handlePhoneClick },
              ...
              <div className="flex flex-col items-center text-center flex-1">
                ...
                <div className="mt-auto pt-2 w-full">
                  <a href={card.buttonLink} onClick={card.buttonAction} className="block w-full py-2.5 px-3 bg-accent/10 hover:bg-accent hover:text-bg-primary text-accent text-sm font-medium rounded-lg transition-all duration-300 text-center" target={card.buttonLink.startsWith('http') ? '_blank' : undefined} rel={card.buttonLink.startsWith('http') ? 'noopener noreferrer' : undefined}>
                    {card.buttonText}<Icon name="fa-arrow-right" className="text-xs ml-2" />
                  </a>
                </div>
              </div>
            </motion.div>
