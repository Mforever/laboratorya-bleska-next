    ...
    <section className="py-20 bg-bg-secondary">
      <div className="container-custom">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <span className="inline-block text-accent font-medium text-sm uppercase tracking-[0.2em] mb-2">Наши услуги</span>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Что мы предлагаем</h2>
          <p className="text-text-secondary max-w-2xl mx-auto text-sm md:text-base">Комплексный уход за вашим автомобилем с использованием премиальных материалов</p>
        </motion.div>
          ...
          {services.map((service, index) => (
            <motion.div key={service.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}
              ...
              <div className="p-5 md:p-6 flex flex-col flex-grow">
                ...
                <p className="text-text-secondary text-sm md:text-base mb-4 min-h-[40px]">{service.description}</p>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/10 flex-wrap gap-3">
                  ...
                  <Button size="small" onClick={() => router.push(service.link)} variant="primary">Подробнее</Button>
                </div>
              </div>
            </motion.div>
          ))}
