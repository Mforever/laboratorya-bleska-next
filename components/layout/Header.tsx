  ...
  return (
    <header
      className={`fixed w-full z-50 transition-all duration-500 ${isScrolled
          ? 'bg-bg-primary/95 backdrop-blur-md py-2 md:py-3 shadow-lg'
        ...
        <nav className="flex items-center justify-between gap-2 md:gap-4">
                ...
                {item.dropdown ? (
                  <>
                    <button
                      ...
                      className={`px-3 xl:px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeDropdown === item.label
                  ...
                  <button
                    ...
                    className={`px-3 xl:px-4 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${pathname === item.path
                        ? 'bg-accent text-bg-primary'
                        : 'text-text-secondary hover:text-accent hover:bg-white/5'
                      }`}
                  >
          ...
          <div className="hidden lg:flex items-center gap-3 xl:gap-4">
            <a
              ...
              className="flex items-center gap-2 text-accent hover:text-accent-hover transition-colors whitespace-nowrap"
            >
              <i className="fas fa-phone-alt text-sm"></i>
              <span className="text-sm font-medium hidden xl:inline">+7 (962) 055-58-58</span>
              <span className="text-sm font-medium xl:hidden">Звонок</span>
            </a>
            <button
              onClick={openBookingModal}
              className="px-4 py-2 bg-accent hover:bg-accent-hover text-bg-primary text-sm font-medium rounded-lg transition-all hover:scale-105 whitespace-nowrap"
            >
              Записаться
            </button>
          </div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="lg:hidden mt-4 bg-bg-secondary rounded-xl overflow-hidden border border-white/5"
            >
              <div className="p-2">
                {menuItems.map((item) => (
                  <div key={item.label}>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                  <a
                    href="tel:+79620555858"
                    onClick={handlePhoneClick}
                    className="flex items-center justify-center gap-3 px-4 py-3 text-accent hover:bg-bg-element rounded-lg transition-colors"
                  >
